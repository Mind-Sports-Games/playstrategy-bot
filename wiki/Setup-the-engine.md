...
- If you want the engine to run in a different directory (e.g., if the engine needs to read or write files at a certain location), enter that directory in the `engine: working_dir` field.
  ...
  - IMPORTANT NOTE: If this field is used, the running engine will look for files and directories (Syzygy tablebases, for example) relative to this path, not the directory where playstrategy-bot was launched. Files and folders specified with absolute paths are unaffected.
...
**Next step**: [Configure playstrategy-bot](https://github.com/Mind-Sports-Games/playstrategy-bot/wiki/Configure-playstrategy-bot)

**Previous step**: [Upgrade to a BOT accout](https://github.com/Mind-Sports-Games/playstrategy-bot/wiki/Upgrade-to-a-BOT-account)