...
def upgrade_account(li: playstrategy.PlayStrategy) -> bool:
...
def watch_control_stream(control_queue: CONTROL_QUEUE_TYPE, li: playstrategy.PlayStrategy) -> None:
...
def start(li: playstrategy.PlayStrategy, user_profile: USER_PROFILE_TYPE, config: Configuration, logging_level: int,
...
def playstrategy_bot_main(li: playstrategy.PlayStrategy,
...
def accept_challenges(li: playstrategy.PlayStrategy, challenge_queue: MULTIPROCESSING_LIST_TYPE, active_games: set[str],
                      max_games: int) -> None:
    """Accept a challenge."""
...
def check_online_status(li: playstrategy.PlayStrategy, user_profile: USER_PROFILE_TYPE, last_check_online_time: Timer) -> None:
...
def game_is_active(li: playstrategy.PlayStrategy, game_id: str) -> bool:
...
def handle_challenge(event: EVENT_TYPE, li: playstrategy.PlayStrategy, challenge_queue: MULTIPROCESSING_LIST_TYPE,
...
def play_game(li: playstrategy.PlayStrategy,
              game_id: str,
              control_queue: CONTROL_QUEUE_TYPE,
              user_profile: USER_PROFILE_TYPE,
              config: Configuration,
...
def should_exit_game(board: chess.Board, game: model.Game, prior_game: Optional[model.Game], li: playstrategy.PlayStrategy,
...
def try_get_pgn_game_record(li: playstrategy.PlayStrategy, config: Configuration, game: model.Game, board: chess.Board,
...
def pgn_game_record(li: playstrategy.PlayStrategy, config: Configuration, game: model.Game, board: chess.Board,
                    engine: engine_wrapper.EngineWrapper) -> str:
    """
...
def start_playstrategy_bot() -> None:
    ...
    li = playstrategy.PlayStrategy(CONFIG.token, CONFIG.url, __version__, logging_level, max_retries)
